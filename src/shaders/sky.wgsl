...
// Ported from https://www.shadertoy.com/view/wcBSzc
...
fn skyRay(org: vec3f, dir: vec3f, sun_direction: vec3f, fast: bool) -> vec3f {
    ...
    let nbSample = select(35, 13, fast); // if fast use 13 else 35
    var color = vec3f(0.0);
...
fn fs_main(input: VertexOutput) -> @location(0) vec4f {
    ...
    if depth >= 1000000.0 { // Use a large value indicating "infinity" or background
        ...
        let org = camera.camera_position;
        ...
        let sun_dir = normalize( vec3f(0.6, 0.45, -0.8) );

        // Call the ported sky rendering function
        let sky_color = skyRay(org, dir, sun_dir, false); // Use 'false' for higher quality

        // Apply some basic exposure/tonemapping if needed
        // Slightly adjusted exposure from original post
        //let final_color = pow(sky_color * 0.04, vec3f(1.0/2.2)); // Example scaling + gamma
