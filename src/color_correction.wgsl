...
fn fs_main(@builtin(position) frag_coord: vec4<f32>) -> @location(0) vec4<f32> {
    ...
    // // Apply brightness
    // color = vec4(color.rgb * cc_uniform.brightness, 0.0);

    // // Apply contrast
    // color = vec4((color.rgb - 0.5) * cc_uniform.contrast + 0.5, 0.0);

    // // Apply saturation
    // let gray = dot(color.rgb, vec3<f32>(0.299, 0.587, 0.114));
    // color = vec4(mix(vec3<f32>(gray), color.rgb, cc_uniform.saturation), 0.0);
