...
impl EntityRenderer {
    pub fn new(
        ...
        let depth_texture_bind_group_layout =
            device.create_bind_group_layout(&wgpu::BindGroupLayoutDescriptor {
                label: Some("Entity Depth Texture Bind Group Layout"),
                entries: &[
        ...
        let entity_shader = shader_hot_reload.get_shader("entity.wgsl");
        // -------------------------------------------

        let render_pipeline =
            Self::create_pipeline(&device, &pipeline_layout, &entity_shader, output_format);
    ...
    pub fn update_depth_texture_bind_group(&mut self, depth_texture_view: &wgpu::TextureView) {
        self.depth_texture_bind_group =
            Some(self.device.create_bind_group(&wgpu::BindGroupDescriptor {
                label: Some("Entity Depth Texture Bind Group"),
    ...
    pub fn check_shader_updates(&mut self, output_format: wgpu::TextureFormat) {
        ...
        if updated_shaders.contains(&"entity.wgsl".to_string()) {
            self.reload_shader(output_format);
        }
    }

    pub fn reload_shader(&mut self, output_format: wgpu::TextureFormat) {
        info!("Reloading entity shader");
        // Get the updated shader module
        let sky_shader = self.shader_hot_reload.get_shader("entity.wgsl");
    ...
    pub fn render<'rpass>(&'rpass self, rpass: &mut wgpu::RenderPass<'rpass>) {
        if let Some(depth_bind_group) = &self.depth_texture_bind_group {
            ...
            tracing::warn!("EntityRenderer depth_texture_bind_group not set, skipping render.");
        }
    }
}
