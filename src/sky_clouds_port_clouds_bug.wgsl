...
fn fs_main(@location(0) texCoord: vec2f) -> @location(0) vec4f {
    ...
    let q = fragCoord / uniforms.resolution;
    var v = -1.0 + 2.0 * q;
    v.x *= uniforms.resolution.x / uniforms.resolution.y;
    
    let org = vec3f(6.0, 2.0, 6.0);
    let ta = vec3f(3.0, 5.0, 3.0);
  ...
  let uu = normalize(cross(vec3f(0.0, 1.0, 0.0), ww));
    ...
    if (fogDistance == -1.0) {
        ...
        var newFogDistance = intersectSphere(org, dir, vec3f(0.0, -EARTH_RADIUS, 0.0), EARTH_RADIUS + 160.0);
