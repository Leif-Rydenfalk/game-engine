use crate::{
    ...
    ColorCorrectionUniform, ImguiState, Model, Transform, VoxelRenderer,
};
...
pub struct CameraUniform {
    ...
    pub inv_view_proj: [[f32; 4]; 4], // offset 64, size 64
    pub view: [[f32; 4]; 4],          // offset 128, size 64
    pub camera_position: [f32; 3],    // offset 192, size 12
    /// Padding after vec3f before f32
    _padding0: u32, // offset 204, size 4 -> next field at 208
    pub time: f32,                    // offset 208, size 4
    /// Padding to make total size multiple of 16
    _padding1: [u8; 12], // offset 212, size 12 -> total size 224
}

const _: () = assert!(std::mem::size_of::<CameraUniform>() == 224);
            ...
            .build(|| {
                ...
                match self.render_mode {
                    RenderMode::Voxel => {
                        ...
                        let mut show_steps = self.voxel_renderer.voxel_settings.show_ray_steps != 0;
                        if ui.checkbox("Show Ray Steps", &mut show_steps) {
                            self.voxel_renderer.voxel_settings.show_ray_steps =
