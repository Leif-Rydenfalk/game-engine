...
fn trace(ro: vec3f, rd: vec3f, tmax: f32) -> HitInfo {
    ...
    for (var i = 0; i < STEPS; i = i + 1) {
        ...
        if (!voxel) {
            ...
            let nrd = select(
                vec3f(0.0, 0.0, srd.z),
                select(vec3f(0.0, srd.y, 0.0), vec3f(srd.x, 0.0, 0.0), t2.x <= t2.y && t2.x <= t2.z),
                t2.y <= t2.z
            );
...
fn fs_main(input: VertexOutput) -> @location(0) vec4f {
    ...
    if (hit.is_hit) {
        ...
        let a = 0.012;
        let b = 0.08;
        let fog = (a / b) * exp(-(ro.y - WATER_HEIGHT) * b) * (1.0 - exp(-t * rd.y * b)) / max(rd.y, EPS);
    ...
    if (pt > 0.0 && (pt < t || ro.y < WATER_HEIGHT)) {
        ...
        if (pt < t) {
            ...
            let foam = smoothstep(0.22, 0.0, wd + sin((wd - camera.time * 0.03) * 60.0) * 0.03 + (wh - 0.5) * 0.12);
