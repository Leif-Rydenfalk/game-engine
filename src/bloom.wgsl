// Define constants once at the top level to avoid redefinition
...
// Shared calc_offset function that takes a texture parameter to avoid redefinition
fn calc_offset(octave: f32, tex: texture_2d<f32>) -> vec2<f32> {
...
@group(1) @binding(0) var horizontal_t_input: texture_2d<f32>;
@group(1) @binding(1) var horizontal_s: sampler;
...
@group(2) @binding(0) var mip_t_input: texture_2d<f32>;
@group(2) @binding(1) var mip_s: sampler;
...
fn mip_fs_main(@location(0) uv: vec2<f32>) -> @location(0) vec4<f32> {
    ...
    color += grab(uv, 2.0, calc_offset(1.0, mip_t_input));
    color += grab(uv, 3.0, calc_offset(2.0, mip_t_input));
    color += grab(uv, 4.0, calc_offset(3.0, mip_t_input));
    color += grab(uv, 5.0, calc_offset(4.0, mip_t_input));
    color += grab(uv, 6.0, calc_offset(5.0, mip_t_input));
    color += grab(uv, 7.0, calc_offset(6.0, mip_t_input));
    color += grab(uv, 8.0, calc_offset(7.0, mip_t_input));
...
@group(3) @binding(0) var apply_t_scene: texture_2d<f32>;
@group(3) @binding(1) var apply_t_bloom: texture_2d<f32>;
@group(3) @binding(2) var apply_s: sampler;

fn get_bloom(coord: vec2<f32>) -> vec3<f32> {
    var bloom = vec3<f32>(0.0);
    bloom += textureSample(apply_t_bloom, apply_s, (coord / exp2(1.0)) - calc_offset(0.0, apply_t_bloom)).rgb * 1.0;
    bloom += textureSample(apply_t_bloom, apply_s, (coord / exp2(2.0)) - calc_offset(1.0, apply_t_bloom)).rgb * 1.5;
    bloom += textureSample(apply_t_bloom, apply_s, (coord / exp2(3.0)) - calc_offset(2.0, apply_t_bloom)).rgb * 1.0;
    bloom += textureSample(apply_t_bloom, apply_s, (coord / exp2(4.0)) - calc_offset(3.0, apply_t_bloom)).rgb * 1.5;
    bloom += textureSample(apply_t_bloom, apply_s, (coord / exp2(5.0)) - calc_offset(4.0, apply_t_bloom)).rgb * 1.8;
    bloom += textureSample(apply_t_bloom, apply_s, (coord / exp2(6.0)) - calc_offset(5.0, apply_t_bloom)).rgb * 1.0;
    bloom += textureSample(apply_t_bloom, apply_s, (coord / exp2(7.0)) - calc_offset(6.0, apply_t_bloom)).rgb * 1.0;
    bloom += textureSample(apply_t_bloom, apply_s, (coord / exp2(8.0)) - calc_offset(7.0, apply_t_bloom)).rgb * 1.0;
    return bloom;
}
...
fn apply_fs_main(@location(0) uv: vec2<f32>) -> @location(0) vec4<f32> {
    var color = textureSample(apply_t_scene, apply_s, uv).rgb;
