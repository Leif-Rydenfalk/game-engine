...
impl Gamepad<'_> {
    ...
    /// Sets the force feedback state directly.
    ///
    /// This method allows directly setting the strength of the rumble motors.
    ///
    /// # Parameters
    ///
    /// * `strong` - Intensity of the strong/low-frequency rumble motor (0 - u16::MAX)
    /// * `weak` - Intensity of the weak/high-frequency rumble motor (0 - u16::MAX)
    /// * `duration` - How long the effect should last
    ///
    /// # Returns
    ///
    /// `Ok(())` if successful, or an error if the operation failed or FF is not supported.
    pub fn set_ff_state<D: Into<Option<Duration>>>(
        &self,
        strong: u16,
        weak: u16,
        duration: D,
        ...
        let duration = duration.into().unwrap_or_else(|| {
            const TICK_DURATION: u32 = 50;
            Duration::from_millis(u64::from(TICK_DURATION) * 2)
        });

        self.data.tx.send(Message::SetFfState {
            id: self.id().0,
            strong,
            weak,
            duration,
